...
class Converts
{
     ...
     * @return int|string
     */
    public static function base_convert($value, $from_base, $to_base) {
        if (is_integer($from_base)) {
            if ($from_base < 37) {
                $value = strtoupper($value);
            }
            $from_base = self::get_alphabet_for_base($from_base);
        }
        if (is_integer($to_base)) {
            $to_base = self::get_alphabet_for_base($to_base);
        }
        return self::base_convert_alphabets($value, $from_base, $to_base);
    }

    public static function get_alphabet_for_base($base) {
     ...
     * @param string $from_alphabet a string of characters used as a base for $value
     * @param string $to_alphabet a string of characters used as a resulting base
     * @return int|string
     */
    public static function base_convert_alphabets($value, $from_alphabet, $to_alphabet) {
        ...
        if ($from_alphabet == $base10_alphabet) {
            ...
            $base10_value = self::base_convert_alphabets($value, $from_alphabet, $base10_alphabet);
        }
        ...
        while ($base10_value != '0') {
            $result = $to_base_chars[self::my_bcmod($base10_value, $to_len)] . $result;
            $base10_value = self::my_bcdiv($base10_value, $to_len, 0);
        }
     ...
     * @return float
     */
    private static function my_bcdiv( $first, $second, $scale = 0 )
    {
        $res = $first / $second;
